...
void CBitVector::Create(size_t bits)
{
	// if( m_pBits ) free(m_pBits);
	size_t size = CEIL_DIVIDE(bits, AES_BITS);
...
void CBitVector::Copy(BYTE* p, size_t pos, size_t len)
{
	if( pos+len > m_nSize)
	{
		if(m_pBits)
...
//pos and len in bits
void CBitVector::SetBits(BYTE* p, size_t pos, size_t len)
{
...
void CBitVector::GetBits(BYTE* p, size_t pos, size_t len)
{
	if(len < 1 || (pos+len) > m_nSize << 3)
		return;
	if(len == 1)
...
void CBitVector::XORBytesReverse(BYTE* p, size_t pos, size_t len)
{
	BYTE* src = p;
...
void CBitVector::XORBitsPosOffset(BYTE* p, int ppos, size_t pos, size_t len)
{
...
void CBitVector::XORBits(BYTE* p, size_t pos, size_t len)
{
	if(len < 1 || (pos+len) > m_nSize << 3)
		return;
	if(len == 1)
...
void CBitVector::ORByte(size_t pos, BYTE p)
{
	m_pBits[pos] |= p;
}
...
void CBitVector::GetBytes(BYTE* p, size_t pos, size_t len)
{
...
void CBitVector::XORBytes(BYTE* p, size_t pos, size_t len)
{
...
void CBitVector::XORRepeat(BYTE* p, size_t pos, size_t len, int num)
{
...
//optimized bytewise for set operation
void CBitVector::SetBytes(BYTE* p, size_t pos, size_t len)
{
...
void CBitVector::ANDBytes(BYTE* p, size_t pos, size_t len)
{
...
void CBitVector::SetXOR(BYTE* p, BYTE* q, size_t pos, size_t len)
{
	Copy(p, pos, len);
	XORBytes(q, pos, len);
}

void CBitVector::SetAND(BYTE* p, BYTE* q, size_t pos, size_t len)
{
	Copy(p, pos, len);
	ANDBytes(q, pos, len);
}
