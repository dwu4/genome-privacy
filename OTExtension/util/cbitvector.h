...
public:
	...
	void Copy(CBitVector& vec, size_t pos, size_t len) { Copy(vec.GetArr(), pos, len); }
	void Copy(BYTE* p, size_t pos, size_t len);
	 ...
	 * Get Operations
	 */
	void GetBits(BYTE* p, size_t pos, size_t len);
	void GetBytes(BYTE* p, size_t pos, size_t len);
	template <class T> void GetBytes(T* dst, T* src, T* lim);
	template <class T> T Get(size_t pos, size_t len)
	{
		T val = 0;
	 ...
	 * Set Operations
	 */
	void SetBits(BYTE* p, size_t pos, size_t len);
	void SetBytes(BYTE* p, size_t pos, size_t len);
	template <class T> void SetBytes(T* dst, T* src, T* lim);
	template <class T> void Set(T val, size_t pos, size_t len) { SetBits((BYTE*) &val, pos, len); }
	 ...
	 * XOR Operations
	 */
	void XORBytes(BYTE* p, size_t pos, size_t len);
	void XORBytes(BYTE* p, size_t len) { XORBytes(p, 0, len); }
	void XORVector(CBitVector &vec, size_t pos, size_t len) { XORBytes(vec.GetArr(), pos, len); }
	template <class T> void XOR(T val, size_t pos, size_t len) { XORBits((BYTE*) &val, pos, len); }
	void XORBits(BYTE* p, size_t pos, size_t len);
	void XORBitsPosOffset(BYTE* p, int ppos, size_t pos, size_t len);
	template <class T> void XORBytes(T* dst, T* src, T* lim);
	void XORRepeat(BYTE* p, size_t pos, size_t len, int num);
	void XORBytesReverse(BYTE* p, size_t pos, size_t len);
	 ...
	 * AND Operations
	 */
	void ANDBytes(BYTE* p, size_t pos, size_t len);
	 ...
	 * Set operations
	 */
	void SetXOR(BYTE* p, BYTE* q, size_t pos, size_t len);
	void SetAND(BYTE* p, BYTE* q, size_t pos, size_t len);
...
private:
	...
	AES_KEY_CTX 	m_nKey;
	BOOL 		m_bKeyInit;
	size_t 		m_nBits; //The exact number of bits
	size_t			m_nElementLength;
	size_t 		m_nNumElements;
	size_t			m_nNumElementsDimB;
};
